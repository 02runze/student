var HerfJson = [{'url':'gdjy','name':'高等教育信息',
    ...
    },{'url':'gjhz、xw、gdxw','name':'出国（境）报告发送',
        'navSecond':[
...
function selectMenu() {
    ...
    if(requestUrl.indexOf("wzy") !== -1 || requestUrl.indexOf('check')!==-1 || requestUrl.indexOf('auth')!==-1 || requestUrl.indexOf('zybh')!==-1){ //核验身份相关页面,或者授权页面，专业变换查询页面
        $('.sub_nav').hide();
...
function supNavHover(){
    ...
    //一级菜单鼠标以上效果
    $("#nav-first dl").mouseover(function(){
       $(this).find('dd').show();
       $('.header-nav-bg').show();
    }).mouseout(function(){
       $(this).find('dd').hide();
